...
int main() {
    ...
    StringBuffer my_buffer(1024); // (O) explicit 생성자를 사용하여 명시적으로 객체를 생성합니다.
    ...
    // processBuffer(50); // (X) explicit 생성자 때문에 컴파일 에러 발생
    processBuffer(StringBuffer(50)); // (O) 명시적으로 객체를 생성하여 전달
