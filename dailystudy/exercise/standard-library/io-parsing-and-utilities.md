...
`std::ostream`은 문자 출력 스트림의 기반 타입이다. `std::cout`과 `std::cerr`도 이 인터페이스를 사용하며, 삽입 연산자 `operator<<`는 보통 `std::ostream&`를 반환한다. 따라서 `std::cout << a << b`는 첫 호출이 돌려준 같은 스트림 참조에 다음 호출을 이어 붙인다. 반환값은 새 스트림이나 복사본이 아니며 스트림 객체의 수명은 호출 전후로 계속 유지된다.

- `cin`은 표준 입력에 연결된 `istream`, `cout`은 일반 표준 출력, `cerr`는 오류 표준 출력에 연결된 `ostream` 객체다.
- `operator>>`는 형식화 입력을 수행하고 스트림 참조를 반환하므로 `cin >> a >> b`처럼 연쇄한다.
