...
## 할당 실패 `std::bad_alloc` — `<new>` 예외 타입

`std::bad_alloc`은 replaceable allocation function이 요청을 만족하지 못할 때 던질 수 있는 표준 예외 타입이다. vector·string 같은 컨테이너 생성이나 성장도 내부 동적 할당 실패를 이 예외로 전달할 수 있다. 호출이 이 타입을 직접 반환하는 것이 아니며, 성공 경로의 반환형·후조건과 예외 경로를 구분해야 한다.

- `what()`은 진단 문자열을 가리키는 `const char*`를 반환하지만 프로그램 로직이 특정 문구에 의존하면 안 된다.
- 예외가 나면 컨테이너가 기존 상태를 유지하는지는 해당 연산의 예외 보장과 원소 타입의 복사·이동 계약에 달린다.
- 무조건 catch해 계속 진행하기보다 더 작은 요청, 오류 전파, 프로세스 정책 중 어느 복구가 실제로 가능한지 경계에서 정한다.
- 다른 스레드의 메모리를 확보하거나 동시 접근을 안전하게 해 주는 기능은 아니며, 구체 allocator·운영체제·overcommit 정책에 따라 실패 시점이 달라질 수 있다.
...
- `operator<<(value)`는 일반 `ostream`처럼 값을 형식화해 동기 버퍼에 추가하고 스트림 참조를 반환한다. 표준은 이 형식 출력에 공통 복잡도 상한을 두지 않으며, 실제 비용은 형식화할 문자·locale·버퍼 구현에 달리고 버퍼 확장에서 할당이 일어날 수 있다.
...
- 항목 종류는 `<string>`의 비멤버 overload 함수 집합이다. 2026-09-21의 선택된 형태는 `std::string to_string(long long value)`다.
- 수신 객체는 없고 `value`는 정수 값으로 복사된다. 오늘 `limit` lvalue의 값은 `[0,10^18]`이며 함수가 외부 문자 저장소나 소유권을 빌리지 않는다.
- 반환형은 새 `std::string` 값이고 `value`의 십진 표현을 소유한다. 오늘 반환 prvalue가 `DigitCounter::digits_`를 직접 초기화하며 입력 limit는 변하지 않는다.
- 표준은 이 함수에 별도 시간·공간 복잡도 상한을 명시하지 않는다. 반환 문자열이 저장 공간을 할당할 수 있고 할당 실패 예외 등이 전파될 수 있다. 호출은 입력 정수, 기존 문자열, 포인터·참조·반복자를 바꾸거나 무효화하지 않는다.
- 반환 문자열의 문자 참조·포인터·반복자 수명은 그 반환 객체와 이후 변경에 묶인다. 오늘은 `DigitCounter`가 문자열을 소유하고 `operator[]` 결과를 즉시 값으로 읽는다. 같은 독립 입력으로 동시에 호출하는 데 공유 사용자 상태는 없지만 구현의 형식 변환 계약만 사용하고 C 로케일 혼용 가정은 피한다.
- 형식과 로케일 세부 제어가 필요하면 `std::format`, 스트림 또는 `to_chars`를 검토한다. 변환 대상 overload, 결과 형식과 성능 요구를 먼저 확인한다.
...
`std::min`과 `std::max`의 대표 계약은 중복하지 않고 [`algorithms-and-ranges.md`](algorithms-and-ranges.md)의 비교·최솟값/최댓값 절에서 관리한다.
